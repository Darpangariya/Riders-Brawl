...
{
    //Edit Var
    public float steerFloat, accelFloat;
    public bool brakeBool, shiftBool;
    ...
    void FixedUpdate()
    {
        ...
        //debugText1.text = speed.ToString();
        ...
        if (activeControl)
        {
            ...
            if (controlMode == ControlMode.simple)
            {
                ...
                {
                    steer = Mathf.MoveTowards(steer, steerFloat, 0.2f); //Input.GetAxis("Horizontal")steer
                    accel = accelFloat;// Input.GetAxis("Vertical");accelFloat
                    brake = brakeBool;// ;brakeBool
                    shift = shiftBool; // Input.GetKey(KeyCode.LeftShift) | Input.GetKey(KeyCode.RightShift) ;shiftBool
